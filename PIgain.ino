...
currentMillis = millis();  //since its not possible to do two caculation or action at the same time and use a delay, the current millis help to do it
...
if (currentMillis - previousMillis >= interval) {
  ...
  Iaction = (Iaction + Error); //The I action is itself plus the error
  CO = (pGain * Error)+(iGain*Iaction); //The control output is the pgain times the error plus the Igain times Iaction
...
// If the push button 1 is trigger, either the Setpoint, the Pgain, and the Igain will appear on the screen and control them by adding value. 
...
// If the push button 2 is trigger, either the Setpoint, the Pgain, and the Igain will appear on the screen and control them by substracting value. 
...
CurrentState3 = digitalRead(PushB3);
 if (LastState3 == 1 && CurrentState3 == 0){
  ...
  lcd.print("     ");
     break;
     case 2: //The third screen show the PV and the Igain
      lcd.setCursor(0, 1);
  lcd.print("PV = ");
  lcd.print(PV);
  lcd.print(" ");
